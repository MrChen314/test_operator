...
__global__ void test_mla_bwd_kernel(
    ...
    if (is_wg3) {
        ...
        // // Read P and dP from TMEM and write to global memory (done by all threads) (done by all threads after WG0/WG3 complete)
        // // P shape: [B_H, B_TOPK] = [128, 64]
        // constexpr int P_ROWS = B_H / 2;  // 64
        // constexpr int P_COLS = B_TOPK;  // 64
        // constexpr int FLOAT2_PER_ROW = P_COLS / 2 / 2;  // 16 float2 per row
        ...
        // if (idx_in_warpgroup == 0 && cta_idx == 0 && blockIdx.x == 0) {
        //     printf("[WG3] finish 4: P read from TMEM and written to global memory\n");
        // }
        ...
        // if (idx_in_warpgroup == 0 && cta_idx == 0 && blockIdx.x == 0) {
        //     printf("[WG3] finish 5: dP read from TMEM and written to global memory\n");
        // }
    }
